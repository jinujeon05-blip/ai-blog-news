...
with st.sidebar:
    ...
    st.markdown("- 10년 차 전문 블로거 스타일의 글과 **썸네일 디자인 가이드**를 생성합니다.")

st.title("✍️ 10년 차 블로그 포스팅 자동화 마스터")
st.markdown("웹 주소나 뉴스 내용을 입력하면, 블로그 글 패키지와 **마크다운 다운로드**, 그리고 **썸네일 기획안**을 뚝딱 만들어 드립니다.")
...
if st.button("🚀 블로그 포스팅 및 썸네일 가이드 생성하기", type="primary"):
    if not api_key:
        ...
        try:
            ...
            with st.spinner("10년 차 블로거 스타일로 글과 썸네일 가이드를 작성하는 중입니다..."):
                # 통합 프롬프트 작성
                prompt = f"""
                당신은 온라인에서 10년 동안 블로그를 운영하며 수많은 방문자를 모아온 '파워 블로거'이자 전문 콘텐츠 크리에이터입니다.
                아래에 제공된 내용(소스)을 바탕으로, 독자들이 끝까지 읽고 공감할 수 있는 고품질 블로그 포스팅 초안과 썸네일 가이드를 작성해주세요.
                톤앤매너: {blog_tone}
